...
func (ccs *Swap) Lock(counterparty string, image string, tokenid string, tokenContract string, lockTime int64) (string, error) {
	...
	args := argArray("TransferFrom", invoker, chaincodeAddress, tokenid)
...
// chaincode.
func (ccs *Swap) Unlock(agreementID string) error {
	var agreement *Agreement
	var err error
	if agreement, err = ccs.getAgreement(agreementID); err != nil {
		return err
...
// contract chaincode.
func (ccs *Swap) Claim(agreementID string, secret string) error {
	var agreement *Agreement
	var err error
	if agreement, err = ccs.getAgreement(agreementID); err != nil {
		return err
	...
	// Invoke token contract to 'unlock' tokens from custom (chaincode) address.
	args := argArray("Transfer", agreement.Counterparty, agreement.TokenId)
	result := caller.stub.InvokeChaincode(agreement.TokenContract, args, "")
	if result.Status != shim.OK {
		return fmt.Errorf("error transferring tokens in contract %s: %s", agreement.TokenContract, result.Message)
	}
...
func (ccs *Swap) getAgreement(agreementID string) (*Agreement, error) {
	var b []byte
	var err error
...
func (ccs *Swap) putAgreement(agreementID string, agreement *Agreement) error {
