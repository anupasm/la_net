...
// Define objectType names for prefixs
const balancePrefix = "balance"
const nftPrefix = "nft"
const approvalPrefix = "approval"
const chaincodename = "nftsc"
...
func (c *TokenERC721Contract) Approve(ctx contractapi.TransactionContextInterface, operator string, tokenId string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get IsApprovedForAll: %v", err)
	}
	if owner != sender && !operatorApproval {
		return "", fmt.Errorf("the sender %s is not the current owner %s nor an authorized operator %s", sender, owner, operator)
	}
	...
	nftKey, err := ctx.GetStub().CreateCompositeKey(nftPrefix, []string{tokenId})
	if err != nil {
		return "", fmt.Errorf("failed to CreateCompositeKey %s: %v", nftKey, err)
	}

	nftBytes, err := json.Marshal(nft)
	if err != nil {
		return "", fmt.Errorf("failed to marshal nftBytes: %v", err)
	}

	err = ctx.GetStub().PutState(nftKey, nftBytes)
	if err != nil {
		return "", fmt.Errorf("failed to PutState for nftKey: %v", err)
	}
...
func (c *TokenERC721Contract) SetApprovalForAll(ctx contractapi.TransactionContextInterface, operator string, approved bool) (bool, error) {
	...
	sender, err := getIdentity(ctx)
	if err != nil {
		return false, fmt.Errorf("failed to get identity : %v", err)
	}
...
func (c *TokenERC721Contract) TransferFrom(ctx contractapi.TransactionContextInterface, from string, to string, tokenId string) (string, error) {
	...
	sender, err := getIdentity(ctx)
	if err != nil {
		return "false", fmt.Errorf("failed to get identity 1: %v", err)
	}
	...
	if owner != sender && operator != sender && !operatorApproval {
		return "false", fmt.Errorf("the sender %s is not the current owner %s nor an authorized operator %s", sender, owner, operator)
	}

	// Check if `from` is the current owner
	if owner != from {
		return "false", fmt.Errorf("the from is not the current owner %s %s", owner, from)
	}
...
func (c *TokenERC721Contract) Initialize(ctx contractapi.TransactionContextInterface, name string, symbol string) (bool, error) {
	...
	if clientMSPID != "Org1MSP" {
		return false, fmt.Errorf("client is not authorized to set the name and symbol of the token %s", clientMSPID)
	}
...
func (c *TokenERC721Contract) MintWithTokenURI(ctx contractapi.TransactionContextInterface, tokenId string, tokenURI string) (*Nft, error) {
	...
	minter, err := getIdentity(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get mint identity %v", err)
	}
...
func (c *TokenERC721Contract) ClientAccountBalance(ctx contractapi.TransactionContextInterface) (int, error) {
	...
	clientAccountID, err := getIdentity(ctx)
	if err != nil {
		return -1, fmt.Errorf("failed to get identity : %v", err)
	}
...
func (c *TokenERC721Contract) ClientAccountID(ctx contractapi.TransactionContextInterface) (string, error) {
	...
	clientAccount, err := getIdentity(ctx)
	if err != nil {
		return "", fmt.Errorf("failed to get identity : %v", err)
	}
...
func getIdentity(ctx contractapi.TransactionContextInterface) (string, error) {
	...
	if err != nil {
		return "false", fmt.Errorf("failed to get identity 2: %v", err)
	}
	chaincode, err := protoutil.InvokedChaincodeName(signedProposal.ProposalBytes)
	if err != nil || chaincode == chaincodename {
...
// 		return "", fmt.Errorf("chaincode spec is nil %v", err)
// 	}

// 	if cis.ChaincodeSpec.ChaincodeId == nil {
// 		return "", fmt.Errorf("chaincode id is nil %v", err)
// 	}
