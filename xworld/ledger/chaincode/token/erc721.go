...
// Token contract for managing CRUD operations
type Token struct {
...
func _readNFT(tokenId string) (*Nft, error) {
	nftKey, err := caller.stub.CreateCompositeKey(nftPrefix, []string{tokenId})
	if err != nil {
		return nil, fmt.Errorf("failed to CreateCompositeKey %s: %v", tokenId, err)
	}

	nftBytes, err := caller.stub.GetState(nftKey)
	if err != nil {
...
func _nftExists(tokenId string) bool {
	nftKey, err := caller.stub.CreateCompositeKey(nftPrefix, []string{tokenId})
	if err != nil {
...
func (c *Token) BalanceOf(owner string) (int, error) {
	...
	iterator, err := caller.stub.GetStateByPartialCompositeKey(balancePrefix, []string{owner})
	if err != nil {
		return -1, err
	}
...
func (c *Token) OwnerOf(tokenId string) (string, error) {

	nft, err := _readNFT(tokenId)
	if err != nil {
...
func (c *Token) Approve(operator string, tokenId string) (bool, error) {
	...
	owner := nft.Owner
	operatorApproval, err := c.IsApprovedForAll(owner, sender)
	if err != nil {
	...
	err = caller.stub.PutState(nftKey, nftBytes)
	if err != nil {
...
// returns {Boolean} Return whether the approval was successful or not
func (c *Token) SetApprovalForAll(operator string, approved bool) (bool, error) {
	...
	approvalKey, err := caller.stub.CreateCompositeKey(approvalPrefix, []string{sender, operator})
	if err != nil {
	...
	err = caller.stub.PutState(approvalKey, approvalBytes)
	if err != nil {
...
func (c *Token) IsApprovedForAll(owner string, operator string) (bool, error) {

	approvalKey, err := caller.stub.CreateCompositeKey(approvalPrefix, []string{owner, operator})
	if err != nil {
		return false, fmt.Errorf("failed to CreateCompositeKey: %v", err)
	}
	approvalBytes, err := caller.stub.GetState(approvalKey)
	if err != nil {
...
func (c *Token) GetApproved(tokenId string) (string, error) {
...
func (c *Token) TransferFrom(from string, to string, tokenId string) (bool, error) {
	...
	balanceKeyFrom, err := caller.stub.CreateCompositeKey(balancePrefix, []string{from, tokenId})
	if err != nil {
		return false, fmt.Errorf("failed to CreateCompositeKey from: %v", err)
	}

	err = caller.stub.DelState(balanceKeyFrom)
	if err != nil {
	...
	balanceKeyTo, err := caller.stub.CreateCompositeKey(balancePrefix, []string{to, tokenId})
	if err != nil {
		return false, fmt.Errorf("failed to CreateCompositeKey to: %v", err)
	}
	err = caller.stub.PutState(balanceKeyTo, []byte{0})
	if err != nil {
...
// Transfer transfers the ownership of a non-fungible token
// from one owner to another owner
// param {String} to The new owner
// param {String} tokenId the non-fungible token to transfer
// returns {Boolean} Return whether the transfer was successful or not

func (c *Token) Transfer(to string, tokenId string) (bool, error) {
	...
	// Remove a composite key from the balance of the current owner
	balanceKeyFrom, err := caller.stub.CreateCompositeKey(balancePrefix, []string{owner, tokenId})
	if err != nil {
		return false, fmt.Errorf("failed to CreateCompositeKey from: %v", err)
	}
...
func (c *Token) TokenURI(tokenId string) (string, error) {
...
func (c *Token) TotalSupply(ctx contractapi.TransactionContextInterface) int {
	...
	iterator, err := caller.stub.GetStateByPartialCompositeKey(nftPrefix, []string{})
	if err != nil {
...
func (c *Token) Mint(tokenId string, tokenURI string) error {
	...
	minter := caller.address

	// Check if the token to be minted does not exist
	exists := _nftExists(tokenId)
	if exists {
	...
	err = caller.stub.PutState(nftKey, nftBytes)
	if err != nil {
		return fmt.Errorf("failed to PutState nftBytes %s: %v", nftBytes, err)
	}
	...
	balanceKey, err := caller.stub.CreateCompositeKey(balancePrefix, []string{minter, tokenId})
	if err != nil {
		return fmt.Errorf("failed to CreateCompositeKey to balanceKey: %v", err)
	}

	err = caller.stub.PutState(balanceKey, []byte{'\u0000'})
	if err != nil {
		return fmt.Errorf("failed to PutState balanceKey %s: %v", nftBytes, err)
	}
...
func (c *Token) Burn(tokenId string) (bool, error) {
...
func (c *Token) ClientAccountBalance(ctx contractapi.TransactionContextInterface) (int, error) {
	...
	return c.BalanceOf(clientAccountID)
}
...
func (c *Token) ClientAccountID(ctx contractapi.TransactionContextInterface) (string, error) {
